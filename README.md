...
墨是维弈阁的调度中枢。你说一句话，墨自动把任务拆解、分发、串联、汇总：

```
用户: "@auto 做一个新功能"

墨: 收到 → 砺(质疑) → 锋(决策) → 枢(拆需求) → 矩+绘(并行审查) → [开发] → 鉴(测试) → 盾(安全) → 完成
```
...
| **多 Agent 模式** | 每个角色是独立 Agent，CodeBuddy 原生调度，支持墨的自动编排 | `.codebuddy/agents/*.md` | 更强的隔离和并行能力 |
...
# 覆盖 Agent 定义（保留 memory/）
for agent in CEO_锋 PM_枢 架构_矩 设计_绘 QA_鉴 安全_盾 财务_算 内容_辞 顾问_隐 合伙人_砺 执事_墨 探索_寻; do
  cp /tmp/weiyige-latest/$agent/SOUL.md .weiyige/$agent/ 2>/dev/null || true
  cp /tmp/weiyige-latest/$agent/IDENTITY.md .weiyige/$agent/ 2>/dev/null || true
...
# 覆盖协议文件 + CodeBuddy Agent
cp /tmp/weiyige-latest/{PROTOCOL,ROUTER,MEMORY,QUICKSTART}.md .weiyige/
cp /tmp/weiyige-latest/agents_for_codebuddy/*.md .codebuddy/agents/
...
[Harness Engineering](https://martinfowler.com/articles/harness-engineering.html)（Birgitta Böckeler, 2026）提出 Agent = Model + Harness，区分前馈（Guides）和反馈（Sensors）。维弈阁据此引入了"计算型检查 + LLM 审查"双层约束。
