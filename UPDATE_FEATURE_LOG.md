...
## v1.6 — 2026-04-16 · Team Qide + 产物验证

**主题：启 · 执事的大脑升级**

启从"能调度"进化到"智能调度"——支持 CodeBuddy Team Qide 运行时，阶段门禁增加产物验证机制。

### 变更

- **启 · 执事 v3.0**：三种编排模式上线（auto / confirm / step）
  - auto：全链路自动执行，仅失败或需要信息时停下
  - confirm：阶段切换时暂停等用户确认
  - step：每个 Agent 后暂停
  - 平台无关——不依赖 CodeBuddy Team Qide 也能工作
- **Team Qide 支持**：启可以利用 CodeBuddy `task` 工具的 `name` / `team_name` 参数，并行 spawn Agent
...
> 调度器的价值不在于它自己能干什么，而在于它能让其他人更高效地干活。启的三种模式让用户可以在"完全信任"和"逐步确认"之间自由选择。
...
- **[H-04] 启串联 Agent 链**：执事_启/SOUL.md 完善编排逻辑，支持无人值守的全链路自动执行
...
> Martin Fowler 文章的核心洞察：Agent = Qidel + Harness。Harness 分两类——前馈（Guides，提前告诉 Agent 该怎么做）和反馈（Sensors，事后检查做得对不对）。维弈阁之前只有前馈（SOUL.md / PROTOCOL.md），没有反馈（计算型检查）。补上这一层，同一模型的成功率就能从 42% 跳到 78%。

---

## v1.4 — 2026-04-14 · 执事 · 启（自动编排器）
...
> 启的设计灵感来自 Sigrid Jin 的 AI Harness 理念——Agent 不需要人肉一个个 @，需要一个调度器把它们串起来。启不写代码、不审方案、不做设计，它只做一件事：确保正确的 Agent 在正确的时机被调用。
...
- **全员 Skills 标准化**：11 个角色（不含启）共 58 张 Skill 卡片，每张包含触发条件、输入、输出、工具、约束、示例
- **新增角色：寻 · 探索**（第 11 个角色）
...
2026-04-13  v1.3    标准化 — 58 张 Skill 卡片 + 探索(寻)角色 + Windows 支持
     ...
     │
2026-04-14  v1.4    编排 — 执事(启)自动编排器上线，从人肉路由到一句话全链路
     │
     │  ← Martin Fowler Harness Engineering 论文
     │
2026-04-15  v1.5    双层约束 — 8 个 Harness 进化，计算型检查 + LLM 审查
     │
2026-04-16  v1.6    智能调度 — 启 v3.0 三模式 + Team Qide + 产物验证
     │
     ▼
    下一步…
```
...
| 自动编排 | 无 | 启·执事（auto/confirm/step 三模式） |
